---
title: "13C_nanoplastics_degradation_analysis"
author: "E. ZEGHAL"
...
calib_curve_1 <- ggplot(data = calib_1, aes(x = area, y = ppm)) +
  ...
  stat_poly_eq(formula = y ~ x + 0, aes(label = after_stat(eq.label)), coef.digits = 7, size= 5) +
...
models_DIC_plot <- ggplot(data = DIC_models, aes(x = time, y = ratio, group = incubation)) +
  facet_grid(~incubation)+
  geom_point( aes(shape = incubation, color = incubation), size = 3)+
  scale_color_manual(values=  safe)+
  geom_smooth(method = "lm",formula = y ~ x , se = F, color = "black", size = 1, linetype = 5) +
  stat_poly_eq(formula = y ~ x , aes(label = after_stat(eq.label)), coef.digits = 10, size= 2) +
  theme_minimal()+
  ylim(0,0.015)+
...
mDIC_PP <- function(x) intercept_linear_reg_PP + x * slope_linear_reg_PE
...
# write_csv(kills_total_C,"kills_total_c.csv")
```
...
experiment_total_C <- experiment |> 
  ...
  mutate(total_C_umol= CO2_umol + mDIC)
# write_csv(experiment_total_C,"experiment_total_C.csv")
```
...
average_excess_kills <- kill_c_fract |> group_by(incubation,time) |> 
  summarise(average_excess_13CO2_umol = mean(excess_13CO2_umol), std_excess_13CO2_umol = sd(excess_13CO2_umol)) |> 
  ungroup()
...
plot_excess_kills <- ggplot(data = average_excess_kills, aes(x = time, y = average_excess_13CO2_umol, group = incubation)) +
  facet_grid(~incubation)+
  geom_point( aes( color = incubation), size = 3)+
  scale_color_manual(values=  safe)+
  geom_smooth(method = "lm",formula = y ~ x , se = F, color = "black", size = 1, linetype = 5) +
  stat_poly_eq(formula = y ~ x , aes(label = after_stat(eq.label)), coef.digits = 10, size= 4) +
  theme_minimal()+
  ylim(0,1)+
  ggtitle("average excess 13-co2 in kill controls", subtitle = "linear regressions") +
  theme(axis.title.y = element_text(face = "bold"),
        axis.title.x = element_text(face = "bold"),
        axis.text.x = element_text(size = 11, face = "bold"),
        axis.text.y = element_text(size = 11, face = "bold"),
...
linear_reg <- lm(average_excess_13CO2_umol ~time , data = average_excess_kills[average_excess_kills$incubation == levels(average_excess_kills$incubation)[i], ])
...
# write_csv(PE_deg,"PE_degradartion.csv")
...
# write_csv(PE_deg_summary,"PE_degradartion_summary.csv")
```

```{r, warning=FALSE, results='hide'}
#| label: PE net degradation plots
...
plot_PE_net <- ggplot(data = PE_deg_summary, aes(x = time, y = average_Net_excess_13C_umol)) +
  ...
  theme_minimal()+
  # ylim(0,1)+
  ggtitle("neto 13Co2 production 13C-PE incubations", subtitle = "") +
  theme(axis.title.y = element_text(face = "bold"),
        axis.title.x = element_text(face = "bold"),
        axis.text.x = element_text(size = 11, face = "bold"),
        axis.text.y = element_text(size = 11, face = "bold"))
...
```{r, warning=FALSE, results='hide'}
#| label: PP net degradation plots

plot_PP_net <- ggplot(data = PP_deg_summary, aes(x = time, y = average_Net_excess_13C_umol)) +
  ...
  theme_minimal()+
  # ylim(0,2)+
  ggtitle("neto 13Co2 production 13C-PP incubations", subtitle = "") +
  theme(axis.title.y = element_text(face = "bold"),
        axis.title.x = element_text(face = "bold"),
        axis.text.x = element_text(size = 11, face = "bold"),
        axis.text.y = element_text(size = 11, face = "bold"))
